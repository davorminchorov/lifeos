...
@section('title', 'Edit Job Offer - LifeOS')
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-[color:var(--color-primary-700)] dark:text-[color:var(--color-dark-600)]">Edit Job Offer</h1>
        ...
        <div class="bg-[color:var(--color-primary-100)] dark:bg-[color:var(--color-dark-200)] shadow-md rounded-lg border border-[color:var(--color-primary-300)] dark:border-[color:var(--color-dark-300)]">
            <form method="POST" action="{{ route('job-applications.offers.update', [$application, $offer]) }}" class="space-y-6 p-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="number" step="0.01" name="base_salary" id="base_salary" required min="0"
                                   value="{{ old('base_salary', $offer->base_salary) }}"
                        ...
                        <div>
                            ...
                            <select name="currency" id="currency" required
                                ...
                                @foreach($currencies as $curr)
                                    <option value="{{ $curr }}" {{ old('currency', $offer->currency) === $curr ? 'selected' : '' }}>{{ $curr }}</option>
                                @endforeach
                            </select>
                            @error('currency')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="benefits" id="benefits" rows="4"
                                  class="mt-1 block w-full px-3 py-2 border border-[color:var(--color-primary-300)] dark:border-[color:var(--color-dark-300)] rounded-md shadow-sm focus:outline-none focus:ring-[color:var(--color-accent-500)] focus:border-[color:var(--color-accent-500)] bg-[color:var(--color-primary-100)] dark:bg-[color:var(--color-dark-200)] text-[color:var(--color-primary-700)] dark:text-[color:var(--color-dark-600)]"
                                  placeholder="Health insurance, 401k match, PTO, remote work policy, etc...">{{ old('benefits', $offer->benefits) }}</textarea>
                        @error('benefits')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="decision_deadline" id="decision_deadline"
                                   value="{{ old('decision_deadline', $offer->decision_deadline?->format('Y-m-d')) }}"
                                   class="mt-1 block w-full px-3 py-2 border border-[color:var(--color-primary-300)] dark:border-[color:var(--color-dark-300)] rounded-md shadow-sm focus:outline-none focus:ring-[color:var(--color-accent-500)] focus:border-[color:var(--color-accent-500)] bg-[color:var(--color-primary-100)] dark:bg-[color:var(--color-dark-200)] text-[color:var(--color-primary-700)] dark:text-[color:var(--color-dark-600)]">
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                @foreach($statuses as $statusOption)
                                    <option value="{{ $statusOption->value }}" {{ old('status', $offer->status->value) === $statusOption->value ? 'selected' : '' }}>
                ...
                <div class="border-t border-[color:var(--color-primary-200)] dark:border-[color:var(--color-dark-300)] pt-6">
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="4"
                                  class="mt-1 block w-full px-3 py-2 border border-[color:var(--color-primary-300)] dark:border-[color:var(--color-dark-300)] rounded-md shadow-sm focus:outline-none focus:ring-[color:var(--color-accent-500)] focus:border-[color:var(--color-accent-500)] bg-[color:var(--color-primary-100)] dark:bg-[color:var(--color-dark-200)] text-[color:var(--color-primary-700)] dark:text-[color:var(--color-dark-600)]"
                                  placeholder="Negotiation notes, important terms, concerns, questions...">{{ old('notes', $offer->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
