...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <x-button href="{{ route('ious.index') }}" variant="secondary">
        ...
        <div class="bg-[color:var(--color-primary-100)] dark:bg-[color:var(--color-dark-200)] shadow-md rounded-lg border border-[color:var(--color-primary-300)] dark:border-[color:var(--color-dark-300)]">
            <form method="POST" action="{{ route('ious.store') }}" class="space-y-6 p-6">
                ...
                <!-- Person Name -->
                <div>
                    <x-form.input name="person_name" label="Person Name" :required="true" placeholder="Enter person's name" />
                </div>

                <!-- Amount and Currency -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-form.input type="number" name="amount" label="Amount" :required="true" step="0.01" min="0.01" placeholder="0.00" />
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-form.input type="date" name="transaction_date" label="Transaction Date" :required="true" :value="old('transaction_date', date('Y-m-d'))" />
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-form.input name="category" label="Category" placeholder="e.g., Loan, Borrowed Item, Service" />
                    </div>
                ...
                <!-- Description -->
                <div>
                    <x-form.input type="textarea" name="description" label="Description" :required="true" rows="3" placeholder="What is this IOU for? (e.g., Borrowed money for car repair, Loan for business, Rent payment)" />
                </div>

                <!-- Notes -->
                <div>
                    <x-form.input type="textarea" name="notes" label="Notes" rows="3" placeholder="Any additional information or reminders (optional)" />
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <x-form.input type="number" name="amount_paid" label="Amount Paid" step="0.01" min="0" :value="old('amount_paid', 0)" placeholder="0.00" />
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t border-[color:var(--color-primary-300)] dark:border-[color:var(--color-dark-300)]">
                    <x-button href="{{ route('ious.index') }}" variant="secondary">Cancel</x-button>
                    <x-button type="submit" variant="primary">Create IOU</x-button>
                </div>
            </form>
        </div>
    </div>
