...
interface UtilityBillEditProps {
...
export default function UtilityBillEdit({ utilityBill }: UtilityBillEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        utility_type: utilityBill.utility_type,
        service_provider: utilityBill.service_provider,
        account_number: utilityBill.account_number ?? '',
        service_address: utilityBill.service_address ?? '',
        bill_amount: String(utilityBill.bill_amount),
        currency: utilityBill.currency ?? 'MKD',
        usage_amount: utilityBill.usage_amount ? String(utilityBill.usage_amount) : '',
        usage_unit: utilityBill.usage_unit ?? '',
        rate_per_unit: utilityBill.rate_per_unit ? String(utilityBill.rate_per_unit) : '',
        bill_period_start: utilityBill.bill_period_start,
        bill_period_end: utilityBill.bill_period_end,
        due_date: utilityBill.due_date,
        payment_status: utilityBill.payment_status,
        payment_date: utilityBill.payment_date ?? '',
        service_plan: utilityBill.service_plan ?? '',
        contract_terms: utilityBill.contract_terms ?? '',
        auto_pay_enabled: utilityBill.auto_pay_enabled,
        budget_alert_threshold: utilityBill.budget_alert_threshold ? String(utilityBill.budget_alert_threshold) : '',
        notes: utilityBill.notes ?? '',
    })

    function handleSubmit(e: FormEvent) {
        e.preventDefault()
        put(`/utility-bills/${utilityBill.id}`)
    }

    return (
        <AppLayout>
            <Head title={`Edit ${formatUtilityType(utilityBill.utility_type)} Bill`} />

            <PageHeader
                title={`Edit ${formatUtilityType(utilityBill.utility_type)} Bill`}
                description={`Update bill from ${utilityBill.service_provider}`}
            >
                <Button variant="outline" asChild>
                    <Link href={`/utility-bills/${utilityBill.id}`}>Back to Details</Link>
                </Button>
            </PageHeader>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="flex justify-end gap-3">
                            <Button type="button" variant="outline" asChild>
                                <Link href={`/utility-bills/${utilityBill.id}`}>Cancel</Link>
                            </Button>
