...
interface ProjectInvestmentEditProps {
...
export default function ProjectInvestmentEdit({ projectInvestment }: ProjectInvestmentEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        name: projectInvestment.name,
        project_type: projectInvestment.project_type ?? '',
        stage: projectInvestment.stage ?? '',
        business_model: projectInvestment.business_model ?? '',
        website_url: projectInvestment.website_url ?? '',
        repository_url: projectInvestment.repository_url ?? '',
        equity_percentage: projectInvestment.equity_percentage != null ? String(projectInvestment.equity_percentage) : '',
        current_value: projectInvestment.current_value != null ? String(projectInvestment.current_value) : '',
        start_date: projectInvestment.start_date ?? '',
        end_date: projectInvestment.end_date ?? '',
        status: projectInvestment.status,
        notes: projectInvestment.notes ?? '',
    })

    function handleSubmit(e: FormEvent) {
        e.preventDefault()
        put(`/project-investments/${projectInvestment.id}`)
    }

    return (
        <AppLayout>
            <Head title={`Edit ${projectInvestment.name}`} />

            <PageHeader title={`Edit ${projectInvestment.name}`} description="Update project investment details">
                <Button variant="outline" asChild>
                    <Link href={`/project-investments/${projectInvestment.id}`}>Back to Details</Link>
                </Button>
            </PageHeader>
