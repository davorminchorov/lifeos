...
const alertVariants = cva(
  "relative w-full rounded-lg border p-4 my-2 [&>svg~*]:pl-7 [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4",
  {
    variants: {
      type: {
...
type AlertType = 'default' | 'error' | 'warning' | 'success';
...
export const Alert = React.forwardRef<HTMLDivElement, AlertProps>(
  ({ className, type = "default", title, message, ...props }, ref) => {
    // Type assertion to make TypeScript happy
    const alertType = (type || 'default') as AlertType;
